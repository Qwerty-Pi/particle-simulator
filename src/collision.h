 ...
 * - pos: The location of the particle
...
inline bool is_wall_collision(Vec2 pos, Vec2 vel, int width, int height, int radius) {
    if (pos.x - radius <= 0 && vel.x < 0) {
        return true;
    } else if (pos.x + radius >= width && vel.x > 0) {
        return true;
    }
    if (pos.y - radius <= 0 && vel.y < 0) {
        return true;
    } else if (pos.y + radius >= height && vel.y > 0) {
        return true;
    }
    return false;
}
 ...
 */
inline bool is_particle_overlap(Vec2 loc1, Vec2 loc2, int radius_sum) {
    double dx = loc2.x - loc1.x;
    double dy = loc2.y - loc1.y;
    double sq_distance = dx * dx + dy * dy;
    return sq_distance <= (radius_sum) * (radius_sum);
}
